...
if __name__ == '__main__':
    ...
    # filename_coil = f'{dir_name}/{axis}/current_rotation_{axis}.xls'
    filename_coil = glob.glob(f'{dir_name}/{axis}/*.xls')[0]
    ...
    for r in r_list:
        # set current
        dataframe_coil = pd.read_excel(excel_coil, sheet_name=f'r{r}', header=0)
        for col in column_names:
            if col not in dataframe_coil.columns:
                dataframe_coil[col] = 0
        print(dataframe_coil.columns)
        ...
        # measured data
        foldernames = [f'{dir_name}/{axis}/r{r}/{i}/' for i in range(1, num_angles+1)]  # glob.glob(f'RQ1nc/Z/r{r}/*/')
        print(foldernames)
        axis_list = []
        current_x_list = []
        current_y_list = []
        ...
        for foldername in foldernames:
            ...
            if len(log_files) == 1:
                ...
                if len(B_dataframe) > 5:
                    ...
                    print(f'measurement len {len(B_dataframe)}')
        ...
        plt.scatter(dataframe['By_measured'], dataframe['Bz_measured'], label='measured')
        plt.scatter(dataframe_coil['By_coil'], dataframe_coil['Bz_coil'], label='coil')
        plt.xlabel('By, G')
        plt.ylabel('Bz, G')
        plt.axis('equal')
        ...
        plt.scatter(dataframe['Bx_measured'], dataframe['Bz_measured'], label='measured')
        plt.scatter(dataframe_coil['Bx_coil'], dataframe_coil['Bz_coil'], label='coil')
        plt.xlabel('Bx, G')
        plt.ylabel('Bz, G')
        # plt.axis('equal')
        ...
        dataframe_B[f'Bx_coil_r{r} (mT)'] = dataframe_coil['Bx_coil'].values
        dataframe_B[f'By_coil_r{r} (mT)'] = dataframe_coil['By_coil'].values
        dataframe_B[f'Bz_coil_r{r} (mT)'] = dataframe_coil['Bz_coil'].values
        dataframe_B[f'B_coil_r{r} (mT)'] = np.sqrt(
            dataframe_coil['Bx_coil'].values ** 2 + dataframe_coil['By_coil'].values ** 2 + dataframe_coil['Bz_coil'].values ** 2)

        dataframe_B[f'Bx_measured_r{r} (mT)'] = dataframe['Bx_measured'].values
        dataframe_B[f'By_measured_r{r} (mT)'] = dataframe['By_measured'].values
        dataframe_B[f'Bz_measured_r{r} (mT)'] = dataframe['Bz_measured'].values
        dataframe_B[f'B_measured_r{r} (mT)'] = dataframe['B_measured'].values

        dataframe_B[f'Bx_measured_std_r{r} (mT)'] = dataframe['Bx_measured_std'].values
        dataframe_B[f'By_measured_std_r{r} (mT)'] = dataframe['By_measured_std'].values
        dataframe_B[f'Bz_measured_std_r{r} (mT)'] = dataframe['Bz_measured_std'].values
        dataframe_B[f'B_measured_std_r{r} (mT)'] = dataframe['B_measured_std'].values

    dataframe_B['Bx_coil_mean (mT)'] = np.mean(dataframe_B[[f'Bx_coil_r{r} (mT)' for r in r_list]].values, axis=1)
    dataframe_B['By_coil_mean (mT)'] = np.mean(dataframe_B[[f'By_coil_r{r} (mT)' for r in r_list]].values, axis=1)
    dataframe_B['Bz_coil_mean (mT)'] = np.mean(dataframe_B[[f'Bz_coil_r{r} (mT)' for r in r_list]].values, axis=1)
    dataframe_B['B_coil_mean (mT)'] = np.mean(dataframe_B[[f'B_coil_r{r} (mT)' for r in r_list]].values, axis=1)

    dataframe_B['Bx_measured_mean (mT)'] = np.mean(dataframe_B[[f'Bx_measured_r{r} (mT)' for r in r_list]].values, axis=1)
    dataframe_B['By_measured_mean (mT)'] = np.mean(dataframe_B[[f'By_measured_r{r} (mT)' for r in r_list]].values, axis=1)
    dataframe_B['Bz_measured_mean (mT)'] = np.mean(dataframe_B[[f'Bz_measured_r{r} (mT)' for r in r_list]].values, axis=1)
    dataframe_B['B_measured_mean (mT)'] = np.mean(dataframe_B[[f'B_measured_r{r} (mT)' for r in r_list]].values, axis=1)
