...
def fit_calibration_odmr(filedir0, filedir1_list, savedir, random_files=False):
    ...
    for idx_axis, axis in enumerate(axis_list):
        ...
        filedir1 = filedir1_list[idx_axis]
        ...
        if random_files:
            ...
            filenames_random = sorted(filenames)
        ...
        print(f'{savedir}/{filedir1}.dat')
        df.to_csv(f'{savedir}/{filedir1}.dat')

    return B_fitted_list_axis
...
if __name__ == '__main__':
    ...
    filedir0 = f'generated_data_clean/phi_axis={phi_tilted}_theta_axis={theta_tilted}/'
    # filenames = glob.glob(f'{filedir}/*.dat')
    # print(len(filenames))

    savedir = f'fitted_B_angle_clean/phi_axis={phi_tilted}_theta_axis={theta_tilted}'

    filedir1_list = [f'axis_idx={idx_axis}' for idx_axis in range(3)]
    print(filedir1_list)

    fit_calibration_odmr(filedir0, filedir1_list, savedir, random_files=False)
    #
    # B_init = CartesianVector(210, 80, 20)
    # init_params = {'B_labx': B_init[0], 'B_laby': B_init[1], 'B_labz': B_init[2],
    #                'glor': 5, 'D': 2870, 'Mz1': 0,
